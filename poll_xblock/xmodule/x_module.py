...
from poll_xblock.core_utils.djangolib.markup import HTML
from poll_xblock.utils.constants import (
...
from poll_xblock.xmodule import block_metadata_utils
from poll_xblock.xmodule.exceptions import ItemNotFoundError
from poll_xblock.xmodule.fields import RelativeTime
from poll_xblock.xmodule.util.builtin_assets import add_webpack_js_to_fragment
...
def block_global_local_resource_url(block, uri):
    ...
    raise NotImplementedError(
        "Applications must monkey-patch this function before using local_resource_url for studio_view")  # lint-amnesty, pylint: disable=line-too-long
...
class MetricsMixin:
    ...
    def handle(self, block, handler_name, request,
               suffix=''):  # lint-amnesty, pylint: disable=missing-function-docstring
        start_time = time.time()
        try:
